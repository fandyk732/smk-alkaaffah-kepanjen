  ...
  if (submitted) {
    return (
      <div className="min-h-screen grid place-items-center bg-slate-50 p-4">
        <div className="max-w-md w-full bg-white rounded-3xl p-8 border shadow-sm text-center space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input 
                ...
                className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition" 
              />
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1.5">Tahun Lulus (Angkatan) *</label>
                <input 
                  type="number" 
                  required
                  value={angkatan}
              ...
              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1.5">Jurusan *</label>
                <select 
                  value={jurusan}
                  onChange={(e) => setJurusan(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition"
                >
                  <option value="Teknik Kendaraan Ringan">TKR (Kendaraan Ringan)</option>
                  <option value="Teknik Sepeda Motor">TSM (Sepeda Motor)</option>
                  <option value="Teknik Elektronika Industri">TEI (Elektronika)</option>
                  <option value="Teknik Komputer Jaringan">TKJ (Komputer Jaringan)</option>
                </select>
              </div>
            </div>

            <div>
              <label className="text-xs font-bold text-slate-700 block mb-1.5">Status Kegiatan Saat Ini *</label>
            ...
            {status === "Bekerja" && (
              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1.5">Jabatan / Posisi Kerja</label>
                <input 
                  type="text" 
                  value={posisi}
            ...
            <div>
              <label className="text-xs font-bold text-slate-700 block mb-1.5">No. WhatsApp Aktif (Opsional)</label>
              <input 
                type="text" 
                value={whatsapp}
                ...
                className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition" 
              />
              <p className="text-[11px] text-slate-400 mt-1">Guna penyaluran info lowongan kerja BKK (jika membutuhkan).</p>
            </div>

            <div>
              <label className="text-xs font-bold text-slate-700 block mb-1.5">Testimoni / Pesan untuk Adik Kelas (Opsional)</label>
              <textarea 
                value={testimoni}
                ...
                className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-sm h-24 resize-none focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition" 
              />
            </div>
